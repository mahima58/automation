    ...
    //   cy.wait(3000);
      ...
      cy.get('@itemData').then((data) => {  // Access fixture data using 'data'
        for (let i = 0; i < 5; i++) {
          ...
          cy.get('[style="margin: 1.9rem 0rem;"] > .button-container > .btn').click(); //add button
        }
        cy.get('.btn-close').click();    //cancel button
        cy.get('.btn.btn-secondary.btn-sm').eq(1).click(); //save draft
      });
    });
  });
